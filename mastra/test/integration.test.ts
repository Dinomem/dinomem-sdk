...
test('AgentMemIntegration constructs with just an apiKey', () => {
  const mem = new AgentMemIntegration({ apiKey: 'sk-fake' })
  assert.equal(mem.name, 'AGENTMEM')
...
test('AgentMemIntegration stores defaults from config', () => {
  const mem = new AgentMemIntegration({
    apiKey:     'sk-fake',
    agentId:    'support-bot',
    scope:      'team',
    workflowId: 'wf-1',
...
test('agentmemMemorize returns a Mastra tool with the expected shape', () => {
  const mem  = new AgentMemIntegration({ apiKey: 'sk-fake', agentId: 'a' })
  const tool = agentmemMemorize(mem)
  assert.equal(tool.id, 'agentmem-memorize')
...
test('agentmemRemember returns a Mastra tool with the expected shape', () => {
  const mem  = new AgentMemIntegration({ apiKey: 'sk-fake', agentId: 'a' })
  const tool = agentmemRemember(mem)
  assert.equal(tool.id, 'agentmem-remember')
  assert.ok(tool.description?.toLowerCase().includes('memory'))
  assert.ok(tool.inputSchema,  'has inputSchema')
  assert.ok(tool.execute,      'has execute')
