...
All notable changes to `@agentmem/claude-agent` will be documented here.
...
- Requires `@agentmem/sdk` `^0.8.0` (was `^0.7.0`). No behavior change in
  this package — the recall hook already dropped hits with
  `relevance_score: null` unconditionally. See
  [`@agentmem/sdk` CHANGELOG](../typescript/CHANGELOG.md) for the
